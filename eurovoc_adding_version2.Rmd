...
```{r}
topic_labels <- fread("English_250_keys .csv",
                          header= FALSE,
                          skip=0,
                          drop = c(2),
                          encoding = "Latin-1")
...
labels_tidy_joined_unigrams <- labels_tidy %>%
  ...
  filter(!is.na(term)) %>% 
  select(topic_id, tokens, term, dist, french, length, profondeur, category, link, bt1, mt, domain)
...
#création des bigrammes et fingerprint. Mais comment conserver l'ordre des tokens ???
